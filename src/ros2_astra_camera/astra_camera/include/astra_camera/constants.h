...
#include <cstdlib>      // Required for general utilities
#include <sys/types.h>  // Required for key_t
...
const double TF_PUBLISH_RATE = 0;     // 0 = Static transform
const double DIAGNOSTICS_PERIOD = 0;
...
const std::string DEFAULT_D2C_MODE = "sw";  // sw = software mode, hw = hardware mode
const float ROS_DEPTH_SCALE = 0.001;
const int TIME_FILTER_LENGTH = 10;

// Product IDs (PIDs) - Critical for Device Recognition
// Astra Series
constexpr static uint32_t ASTRA_PRO_PID = 0x0601;       // Standard Astra Pro
constexpr static uint32_t ASTRA_PRO_PLUS_PID = 0x0602;  // Astra Pro Plus
constexpr static uint32_t ASTRA_PRO_UVC_PID = 0x0501;   // UVC Color Stream
...
// IPC and Shared Memory Constants
const std::string DEFAULT_SEM_NAME = "astra_device_sem";
const key_t DEFAULT_SEM_KEY = 0x0401; // Requires sys/types.h
